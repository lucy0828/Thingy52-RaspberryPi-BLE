...
//static uuid_t euler_uuid;
//static uuid_t raw_uuid;
...
void notification_handler(const uuid_t* uuid, const void* data, size_t data_length, struct Packet* p) {
	...
	if(!strcmp(uuid_str, UUID_ENV_TEMP)) {
		...
		p->tvoc = *(uint16_t*)(data+2);
		p->cnt_gas = 1;
	}
	else if(!strcmp(uuid_str, UUID_ENV_COLOR)) {
		p->red = *(uint16_t*)data;
		p->green = *(uint16_t*)(data+2);
		p->blue = *(uint16_t*)(data+4);
		p->clear = *(uint16_t*)(data+6);
		p->cnt_color = 1;
	}
	else {
		...
		if(p->cnt_humd > 0) {
			if(p->cnt_gas > 0) {
				if(p->cnt_color > 0) {
					...
					printf("%d-%d-%d %d:%d:%d,%s,%d.%u,%u,%u,%u,%u,%u,%u,%u\n", tm.tm_year+1900, tm.tm_mon+1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, 
					p->name, p->temp_integer, p->temp_decimal, p->humidity, p->eco2, p->tvoc, p->red, p->green, p->blue, p->clear);
					p->cnt_temp--;
					p->cnt_humd--;
					p->cnt_gas--;
...
static void *ble_connect_device(void *arg) {
	...
	fputs("DateTime,Name,Temperature,Humidity,CO2,Red\n", fp);
	fclose(fp);
